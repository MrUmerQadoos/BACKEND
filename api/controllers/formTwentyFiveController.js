...
export const createFormTwentyFive = async (req, res) => {
  try {
    ...
    // Handle file uploads for alternativeWallsPhotos
    if (req.files && req.files.alternativeWallsPhotos) {
      const alternativeWallsPhotos = Array.isArray(req.files.alternativeWallsPhotos)
        ? req.files.alternativeWallsPhotos
        : [req.files.alternativeWallsPhotos];
      for (const file of alternativeWallsPhotos) {
        const result = await new Promise((resolve, reject) => {
          const stream = cloudinary.uploader.upload_stream(
            { folder: `user-${userId}/form-twenty-five` },
            (error, result) => {
        ...
        filePaths.alternativeWallsPhotos.push(result.secure_url);
      }
    }

    // Handle file uploads for wallInsulationPhotos
...
export const updateFormTwentyFive = async (req, res) => {
  try {
    ...
    let updatedAlternativeWallsPhotos = existingForm.alternativeWallsPhotos.filter(
      (photo) => !deletedAlternativeWallsPhotos.includes(photo)
    );
    ...
    const deleteImage = async (url) => {
      if (url) {
        ...
        const folderPath = `user-${userId}/form-twenty-five`;
        await cloudinary.uploader.destroy(`${folderPath}/${publicId}`);
      }
    };

    await Promise.all(deletedAlternativeWallsPhotos.map(deleteImage));
    await Promise.all(deletedWallInsulationPhotos.map(deleteImage));

    // Process new file uploads for alternativeWallsPhotos
    ...
    if (req.files && req.files.alternativeWallsPhotos) {
      ...
      for (const file of alternativeWallsPhotos) {
        ...
        newAlternativeWallsPhotos.push(result.secure_url);
      }
    }

    // Process new file uploads for wallInsulationPhotos
